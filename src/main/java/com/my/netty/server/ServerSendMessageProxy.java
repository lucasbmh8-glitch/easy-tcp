...
public class ServerSendMessageProxy {

    private static final Gson sendGson = new Gson();

    private static final Logger log = LoggerFactory.getLogger(ServerSendMessageProxy.class);
    ...
    private static final DelayQueue<ServerDelayRetryTask> retryQueue = new DelayQueue<>();
    ...
    static {
        ...
        retryExecutor.submit(() -> {
            while (!Thread.currentThread().isInterrupted()) {
                try {
                    ServerDelayRetryTask task = retryQueue.take();
                    handleRetry(task);
        ...
        cleanupScheduler.scheduleAtFixedRate(() -> {
            ...
            for (Iterator<Map.Entry<String, Long>> it = ServerContent.PENDING_ACK_MESSAGES.entrySet().iterator(); it.hasNext(); ) {
    ...
    public static void sendBusinessMessage(ActivityMsgVo vo, Channel channel) {
        ServerContent.PENDING_ACK_MESSAGES.put(vo.getMsgId(), System.currentTimeMillis());
        ...
        retryQueue.offer(new ServerDelayRetryTask(vo.getMsgId(), message, 1, 2, TimeUnit.SECONDS,channel));

    }

    private static void handleRetry(ServerDelayRetryTask task) {
        String msgId = task.getMsgId();

        // 如果ACK已收到，停止重试
        if (!ServerContent.PENDING_ACK_MESSAGES.containsKey(msgId)) {
            return;
        }

        if (task.getRetryCount() > MAX_ACK_BROADCAST_RETRY) {
            ServerContent.PENDING_ACK_MESSAGES.remove(msgId);
            return;
        }

        // 重试时更新时间戳
        ServerContent.PENDING_ACK_MESSAGES.put(msgId, System.currentTimeMillis());
        ...
        // 继续加入队列进行下一轮重试
        retryQueue.offer(new ServerDelayRetryTask(msgId, task.getMessage(), task.getRetryCount() + 1, 2, TimeUnit.SECONDS,task.getChannel()));
    }
